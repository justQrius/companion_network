"""MCP Client for Bob's Companion to call tools on Alice's Companion MCP server.

This module implements an MCP client that connects to Alice's Companion's MCP endpoint
...
ALICE_ENDPOINT = "http://localhost:8001/run"
...
class MCPClient:
    ...
    It connects to Alice's Companion endpoint and can call tools on her MCP server.
    ...
    def __init__(self, endpoint: str = ALICE_ENDPOINT):
        """Initialize MCP client with target endpoint.
        
        Args:
            endpoint: HTTP endpoint URL for remote MCP server (default: Alice's endpoint)
        """
