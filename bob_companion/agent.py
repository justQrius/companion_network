...
7. **Event Proposal to User**: After coordinating mutual availability and getting a recommendation,
   you should propose the event to Bob for confirmation using the propose_event_to_user() helper function.
   This function:
   ...
   - Stores the proposal in session state as "pending" awaiting Bob's confirmation
   - Presents the proposal message to Bob with a call to action
   - Example: After coordinate_mutual_availability() succeeds, call propose_event_to_user(recommendation, "alice")
   - The proposal waits for Bob's next message ("yes", "confirm", "sounds good") to proceed

8. **Acknowledge Naturally**: Respond with natural language acknowledgment that shows understanding:
...
9. **Identify Contact Need**: Determine which Companion agent you need to contact for coordination
   (e.g., if Bob mentions "Alice", you'll need to contact Alice's Companion).

10. **User Confirmation Handling**: When you present an event proposal to Bob, wait for his confirmation
    in his next message. Use the handle_user_confirmation() helper function to check if Bob's response
    indicates approval. Confirmation keywords include: "yes", "confirm", "sounds good", "go ahead", "proceed".
    If Bob confirms, you can proceed with calling Alice's Companion via A2A to finalize the event.
...
# Agent and runner will be created after tool definitions (see end of file)
agent = None
runner = None
...
async def _check_alice_server_health() -> bool:
    """Check if Alice's MCP server is accessible.
...
async def call_other_companion_tool(tool_name: str, **params) -> dict:
    ...
    Pre-flight check: Verifies Alice's server is accessible before attempting tool call.
    ...
    # Pre-flight check: Verify Alice's server is accessible
    if not await _check_alice_server_health():
        error_result = {
            "error": "Alice's Companion is not available. Please ensure Alice's Companion MCP server is running on localhost:8001.",
            "details": "Health check failed for http://localhost:8001/health"
        }
        logger.error(f"Pre-flight check failed: Alice's server not accessible before calling {tool_name}")
    ...
    try:
        ...
        error_msg = "Agent temporarily unavailable: Cannot connect to Alice's Companion. Please ensure Alice's Companion is running."
...
async def coordinate_mutual_availability(
    ...
    logger.info(f"Attempting A2A call to Alice's Companion: check_availability(timeframe={timeframe})")
    ...
    if "error" in alice_result:
        error_msg = alice_result["error"]
        logger.warning(f"A2A call to Alice's Companion failed: {error_msg}")
        return {
            "success": False,
            "error": error_msg,
    ...
    logger.info(f"A2A call to Alice's Companion succeeded: {len(alice_result.get('slots', []))} slots found")
...
async def create_event_proposal(
    ...
    Args:
        ...
        recipient_user_id: User ID of the person receiving the proposal (default: "alice")
        proposer_user_id: User ID of the person proposing (default: "bob")
...
def format_proposal_message(event_proposal: EventProposal) -> str:
    ...
    context_parts = []
    alice_preferences = details.get("alice_preferences", {})
    cuisine_prefs = alice_preferences.get("cuisine", [])
    if cuisine_prefs:
...
async def handle_user_confirmation(
    ...
    Args:
        ...
        proposer_user_id: User ID of the person who proposed (default: "bob")
    ...
    if is_confirmed:
        # Update proposal status to "user_confirmed" (intermediate status)
        updated_proposal = pending_proposal.copy()
        updated_proposal["status"] = "user_confirmed"
...
async def finalize_event_with_other_companion(
    ...
    - Uses call_other_companion_tool() from Story 2.8 to call Alice's propose_event tool
    ...
    - Updates EventProposal status based on Alice's response
    ...
    # Call Alice's propose_event MCP tool via A2A
    ...
    # Extract response from Alice's Companion
    ...
    # Update EventProposal status based on Alice's response
    session = await session_service.get_session(
        app_name="companion_network",
        user_id=proposer_user_id,
        session_id=SESSION_ID
