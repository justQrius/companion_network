...
async def relay_message(
    ...
    """Relay a message from a trusted contact to Alice.
    
    This tool allows trusted contacts to send messages to Alice through her Companion.
    Messages are queued in session state and displayed to Alice in her next agent interaction.
    The tool validates the sender is in Alice's trusted_contacts list before accepting the message.
    
    Args:
        message: The message text to relay to Alice (required, non-empty string)
    ...
    Message Storage:
        ...
        Messages are displayed to Alice in her next agent response, sorted by urgency priority.
    """
    # AC1: Input validation
